...
class richmenu {
  ...
  getRichMenuImage (richMenuId) {
    return new Promise((resolve, reject) => {
      if (!fs.existsSync(`./public/images/${richMenuId}.png`)) {
        this.axios.get(`https://api.line.me/v2/bot/richmenu/${richMenuId}/content`, { responseType: 'stream' }).then(res => {
          ...
          stream.on('finish', () => {
            console.log(`將LINE伺服器上的圖片下載於 => /public/images/${richMenuId}.png`)
            resolve(`/public/images/${richMenuId}.png`)
          })
        }).catch(err => {
          // console.log(err)
          reject(err)
  ...
  cancelDefaultRichMenu () {
    return new Promise((resolve, reject) => {
      this.axios.delete(`https://api.line.me/v2/bot/user/all/richmenu`).then(() => {
        resolve()
      }).catch(err => {
        console.log(err.response.data)
        reject()
